...
class ImageAttachment(BaseModel):
    """Model for image attachment."""

    data: str = Field(..., description="Base64 encoded image data")
    mediaType: str = Field(
        ..., description="Image media type (e.g., 'image/jpeg', 'image/png')"
    )
...
class DisputeAnalysisRequest(BaseModel):
    ...
    transaction_id: Optional[str] = Field(
        None, description="Optional transaction ID associated with the dispute"
    )
    amount: Optional[float] = Field(None, description="Disputed amount", ge=0)
    session_id: Optional[str] = Field(
        None, description="Session ID for multi-turn conversation"
    )
    additional_evidence: Optional[Dict[str, Any]] = Field(
        None, description="Additional evidence provided in response to agent request"
    )
    images: Optional[List[ImageAttachment]] = Field(
        None,
        description="Optional array of images to support the dispute (e.g., photos of damaged product)",
    )
...
class DisputeAnalysisResponse(BaseModel):
    ...
    evidence_requested: Optional[EvidenceRequest] = Field(
        None, description="Evidence being requested"
    )

    # For completed status
    decision: Optional[DisputeDecision] = Field(
        None, description="Final decision on dispute"
    )

    # General
    message: str = Field(..., description="Message to display")
    step: int = Field(
        default=1, description="Current step in the process (1-2 follow-ups)"
    )
